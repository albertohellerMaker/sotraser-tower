    ...
    <div className="space-y-5" data-testid="excesos-velocidad-page">
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="dash-card px-4 py-3" data-testid="kpi-camiones-exceso">
          <div className="font-exo text-[8px] tracking-[0.15em] uppercase" style={{ color: "#3a6080" }}>CAMIONES CON EXCESOS</div>
          <div className="font-space text-2xl font-bold" style={{ color: totals.camionesConExceso > 0 ? "#ff2244" : "#00ff88" }}>
            {totals.camionesConExceso}
          </div>
          <div className="font-exo text-[8px]" style={{ color: "#3a6080" }}>actualmente sobre limite</div>
        </div>
        <div className="dash-card px-4 py-3" data-testid="kpi-eventos-total">
          <div className="font-exo text-[8px] tracking-[0.15em] uppercase" style={{ color: "#3a6080" }}>EVENTOS TOTALES</div>
          <div className="font-space text-2xl font-bold" style={{ color: "#ffcc00" }}>{totals.eventosTotal}</div>
          <div className="font-exo text-[8px]" style={{ color: "#3a6080" }}>detecciones activas</div>
        </div>
        <div className="dash-card px-4 py-3" data-testid="kpi-mas-grave">
          <div className="font-exo text-[8px] tracking-[0.15em] uppercase" style={{ color: "#3a6080" }}>MAS GRAVE</div>
          {totals.masGrave ? (
            <>
              <div className="font-space text-2xl font-bold" style={{ color: "#ff2244" }}>{totals.masGrave.velocidad} km/h</div>
              <div className="font-exo text-[8px]" style={{ color: "#3a6080" }}>{totals.masGrave.patente}</div>
            </>
          ) : (
        ...
        <div className="dash-card px-4 py-3" data-testid="kpi-hoy">
          <div className="font-exo text-[8px] tracking-[0.15em] uppercase" style={{ color: "#3a6080" }}>HOY</div>
          <div className="font-space text-2xl font-bold" style={{ color: totals.hoy > 0 ? "#ffcc00" : "#00ff88" }}>{totals.hoy}</div>
          <div className="font-exo text-[8px]" style={{ color: "#3a6080" }}>eventos detectados</div>
        </div>
      </div>

      <div className="flex items-center gap-2">
        <button
          className="font-space text-[10px] font-bold tracking-[0.1em] uppercase px-4 py-1.5 transition-all"
          style={{
      ...
      {vista === "camion" ? (
          ...
          {porCamion.length === 0 ? (
            ...
            <>
              <div className="flex items-center gap-3 px-5 py-2" style={{ borderBottom: "1px solid #0d2035", background: "#020508" }}>
                ...
                <span className="font-exo text-[8px] tracking-[0.15em] uppercase" style={{ color: "#3a6080", width: "120px" }}>CONDUCTOR</span>
                <span className="font-exo text-[8px] tracking-[0.15em] uppercase" style={{ color: "#3a6080", width: "120px" }}>FAENA</span>
                <span className="font-exo text-[8px] tracking-[0.15em] uppercase text-right" style={{ color: "#3a6080", width: "80px" }}>VEL. MAX</span>
                <span className="font-exo text-[8px] tracking-[0.15em] uppercase text-right" style={{ color: "#3a6080", width: "70px" }}>EXCESOS</span>
                <span className="font-exo text-[8px] tracking-[0.15em] uppercase flex-1 text-right" style={{ color: "#3a6080" }}>RIESGO</span>
              </div>
              {porCamion.map(c => (
                <div key={c.patente}>
                  <div
                  ...
                  >
                    ...
                    <span className="font-exo text-[10px] truncate" style={{ color: "#c8e8ff", width: "120px" }}>{c.conductor || "---"}</span>
                    <span className="font-exo text-[10px] truncate" style={{ color: "#3a6080", width: "120px" }}>{c.faena || "---"}</span>
                    <span className="font-space text-[12px] font-bold text-right" style={{
                      color: c.velocidadMaxima > 120 ? "#ff2244" : c.velocidadMaxima > 100 ? "#ffcc00" : "#c8e8ff",
                      width: "80px"
                    }}>
                  ...
                  {expandedPatente === c.patente && (
                    <div className="px-5 py-3" style={{ background: "rgba(0,212,255,0.02)", borderBottom: "1px solid #0d2035" }}>
                      <div className="font-exo text-[8px] tracking-[0.15em] uppercase mb-2" style={{ color: "#3a6080" }}>EVENTOS DE EXCESO</div>
                      {c.eventos.map((e, i) => (
                        <div key={i} className="flex items-center gap-3 py-1.5" style={{ borderBottom: "1px solid #0d203520" }}>
                          <span className="font-space text-[10px]" style={{ color: "#3a6080", width: "130px" }}>
                            {new Date(e.fecha).toLocaleString("es-CL")}
                          </span>
                          <span className="font-space text-[12px] font-bold" style={{
                            color: e.velocidadMaxima > 120 ? "#ff2244" : e.velocidadMaxima > 100 ? "#ffcc00" : "#c8e8ff"
                          }}>
                            {e.velocidadMaxima} km/h
                          </span>
                          {e.lat && e.lng && (
                            <span className="font-space text-[8px]" style={{ color: "#3a6080" }}>
        ...
        <div className="dash-card overflow-hidden">
          {eventos.length === 0 ? (
            ...
            <>
              <div className="flex items-center gap-3 px-5 py-2" style={{ borderBottom: "1px solid #0d2035", background: "#020508" }}>
                <span className="font-exo text-[8px] tracking-[0.15em] uppercase" style={{ color: "#3a6080", width: "130px" }}>FECHA</span>
                <span className="font-exo text-[8px] tracking-[0.15em] uppercase" style={{ color: "#3a6080", width: "70px" }}>PATENTE</span>
                <span className="font-exo text-[8px] tracking-[0.15em] uppercase" style={{ color: "#3a6080", width: "120px" }}>CONDUCTOR</span>
                <span className="font-exo text-[8px] tracking-[0.15em] uppercase text-right" style={{ color: "#3a6080", width: "80px" }}>VELOCIDAD</span>
                <span className="font-exo text-[8px] tracking-[0.15em] uppercase" style={{ color: "#3a6080", width: "100px" }}>UBICACION</span>
                <span className="font-exo text-[8px] tracking-[0.15em] uppercase flex-1 text-right" style={{ color: "#3a6080" }}>RIESGO</span>
              </div>
              {eventos.map((e, i) => (
                <div key={i} className="flex items-center gap-3 px-5 py-2.5"
                  ...
                  <span className="font-exo text-[10px] truncate" style={{ color: "#c8e8ff", width: "120px" }}>{e.conductor || "---"}</span>
                  <span className="font-space text-[12px] font-bold text-right" style={{
                    color: e.velocidadMaxima > 120 ? "#ff2244" : e.velocidadMaxima > 100 ? "#ffcc00" : "#c8e8ff",
                    width: "80px"
                  }}>
                  ...
                  <span className="font-space text-[8px]" style={{ color: "#3a6080", width: "100px" }}>
      ...
      <div className="dash-card px-5 py-4">
        <div className="font-exo text-[8px] tracking-[0.15em] uppercase mb-3" style={{ color: "#3a6080" }}>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="font-exo text-[10px] ml-2" style={{ color: "#3a6080" }}>camiones nunca excedieron {totals.limiteKmh} km/h</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="font-exo text-[10px] ml-2" style={{ color: "#3a6080" }}>con excesos ocasionales</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="font-exo text-[10px] ml-2" style={{ color: "#3a6080" }}>con excesos frecuentes</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="text-[9px] font-space tracking-[0.15em]" style={{ color: "#3a6080" }}>
        Datos en tiempo real via Volvo rFMS &middot; Limite configurable en parametros &middot; Auto-refresh cada 60s &middot; ALTO: &gt;120 km/h o &gt;10 eventos &middot; MEDIO: 100-120 km/h &middot; BAJO: 90-100 km/h
      </div>
    </div>
  );
}
