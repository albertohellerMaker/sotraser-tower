  ...
  return (
    <div style={{ background: "#060d14", border: "1px solid #00d4ff30", borderTop: "2px solid #00d4ff", borderRadius: 8 }}>
      <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: "1px solid #0d2035" }}>
        <div className="flex items-center gap-2">
          <span className="font-space text-[11px] font-bold tracking-wider" style={{ color: "#00d4ff" }}>SISTEMA MULTI-AGENTE</span>
        ...
        <button onClick={() => { fetch("/api/agentes/mensajes/leer-todos", { method: "POST" }).then(() => refetch()); }} className="font-exo text-[8px] cursor-pointer" style={{ color: "#3a6080" }}>Marcar leido</button>
      </div>
      <div className="grid grid-cols-8 gap-1.5 p-3" style={{ borderBottom: "1px solid #0d2035" }}>
        {(data?.agentes || []).map((a: any) => {
          const min = a.ultimo_ciclo ? Math.round((Date.now() - new Date(a.ultimo_ciclo).getTime()) / 60000) : null;
          const ok = min !== null && min < 30;
          return (
            <div key={a.id} className="text-center p-1.5" style={{ background: "#0a1520", borderTop: `2px solid ${ok ? colorTipo(a.tipo) : "#3a6080"}`, borderRadius: 4 }}>
              <div className="font-exo text-[7px] uppercase font-bold" style={{ color: ok ? colorTipo(a.tipo) : "#3a6080" }}>{a.nombre?.split(" ")[0]}</div>
      ...
      <div className="overflow-auto" style={{ maxHeight: 180 }}>
        {msgs.slice(0, 5).map((m: any) => (
          <div key={m.id} onClick={() => { fetch(`/api/agentes/mensajes/${m.id}/leer`, { method: "POST" }).then(() => refetch()); }}
            className="px-4 py-2 border-b cursor-pointer transition-all hover:bg-[rgba(255,255,255,0.02)]" style={{ borderColor: "#0a1520", borderLeft: !m.leido ? "3px solid #00d4ff" : "3px solid transparent" }}>
            <div className="flex items-center gap-2">
              ...
              {m.prioridad === "CRITICA" && <span className="font-exo text-[7px] font-bold" style={{ color: "#ff2244" }}>CRITICO</span>}
            </div>
            <div className="font-exo text-[9px] font-bold mt-1 truncate" style={{ color: "#c8e8ff" }}>{m.titulo}</div>
            <div className="font-exo text-[8px] mt-0.5 line-clamp-1" style={{ color: "#3a6080" }}>{m.contenido?.substring(0, 100)}</div>
          </div>
        ))}
