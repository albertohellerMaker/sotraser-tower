...
export async function cruzarConSigetra(): Promise<number> {
  ...
  const sinCruzarR = await pool.query(`
    SELECT v.id, v.camion_id, v.vin, v.fecha_inicio, v.fecha_fin,
           v.litros_consumidos_ecu, c.patente, c.num_veh
  ...
  if (sinCruzarR.rows.length === 0) {
    const pendientesR = await pool.query(`
      SELECT COUNT(*)::int as cnt FROM viajes_aprendizaje
      WHERE sigetra_cruzado = false AND procesado_aprendizaje = true AND fecha_inicio > $1
    `, [limite72h]);
    const pendientes = pendientesR.rows[0]?.cnt || 0;
    if (pendientes > 0) {
      console.log(`[CUADRATURA] ${pendientes} viajes recientes esperando 72h para cruzar con Sigetra`);
    }
    return 0;
  }

  console.log(`[CUADRATURA] Cruzando ${sinCruzarR.rows.length} viajes (>72h) con Sigetra...`);

  const fechaMin = new Date(sinCruzarR.rows[0].fecha_inicio);
  const fechaMax = new Date(sinCruzarR.rows[sinCruzarR.rows.length - 1].fecha_fin || sinCruzarR.rows[sinCruzarR.rows.length - 1].fecha_inicio);
  fechaMin.setDate(fechaMin.getDate() - 2);
  fechaMax.setDate(fechaMax.getDate() + 2);

  let sigetraData: any[] = [];
  try {
    sigetraData = await getCachedFuelData(fechaMin, fechaMax);
  } catch (err: any) {
    console.error("[CUADRATURA] Error cargando Sigetra:", err.message);
    return 0;
  }

  if (sigetraData.length === 0) {
    console.log("[CUADRATURA] Sin datos Sigetra en rango, marcando como cruzados sin match");
    for (const v of sinCruzarR.rows) {
      await pool.query(`UPDATE viajes_aprendizaje SET sigetra_cruzado = true, fecha_cruce_sigetra = NOW() WHERE id = $1`, [v.id]);
    }
    return 0;
  }

  const numVehToPatente = new Map<string, string>();
  const allCamR = await pool.query(`SELECT patente, num_veh FROM camiones WHERE num_veh IS NOT NULL`);
  for (const row of allCamR.rows) {
    numVehToPatente.set(String(row.num_veh), row.patente);
  }

  const cargasByPatente = new Map<string, any[]>();
  for (const carga of sigetraData) {
    const numVeh = String(carga.numVeh || "");
    const pat = carga.patente || numVehToPatente.get(numVeh) || numVeh;
    if (!cargasByPatente.has(pat)) cargasByPatente.set(pat, []);
    cargasByPatente.get(pat)!.push(carga);
  }

  const usedCargas = new Set<string>();
  let cruzados = 0;

  for (const viaje of sinCruzarR.rows) {
    const patente = viaje.patente;
    const cargas = cargasByPatente.get(patente) || [];
    const midpoint = new Date((new Date(viaje.fecha_inicio).getTime() + new Date(viaje.fecha_fin).getTime()) / 2);

    let bestCarga: any = null;
    let bestDist = Infinity;
    let bestKey = "";

    for (const carga of cargas) {
      const key = `${patente}_${carga.fechaConsumo}_${carga.cantidadLt}`;
      if (usedCargas.has(key)) continue;
      const fechaCarga = new Date(carga.fechaConsumo);
      const dist = Math.abs(fechaCarga.getTime() - midpoint.getTime());
      if (dist <= 24 * 60 * 60 * 1000 && dist < bestDist) {
        bestDist = dist;
        bestCarga = carga;
        bestKey = key;
      }
    ...
    if (bestCarga && bestKey) {
      usedCargas.add(bestKey);
      litrosSigetra = bestCarga.cantidadLt || 0;
      const kmAct = bestCarga.odometroActual || 0;
      const kmAnt = bestCarga.odometroPrevio || 0;
      if (kmAct > 0 && kmAnt > 0) kmSigetra = kmAct - kmAnt;
      if (bestCarga.nombreConductor) conductor = bestCarga.nombreConductor;
    }

    const litrosEcu = parseFloat(viaje.litros_consumidos_ecu) || 0;
    const delta = litrosSigetra > 0 ? litrosSigetra - litrosEcu : null;

    await pool.query(`
      UPDATE viajes_aprendizaje SET
        sigetra_cruzado = true,
        fecha_cruce_sigetra = NOW(),
        litros_cargados_sigetra = $1,
        km_declarado_sigetra = $2,
        conductor = COALESCE(conductor, $3),
        delta_cuadratura = $4
      WHERE id = $5
    `, [litrosSigetra, kmSigetra, conductor, delta, viaje.id]);

    if (litrosSigetra > 0) cruzados++;

    if (conductor) {
      await actualizarConductorDesdecruce(conductor, viaje);
    }
  }

  console.log(`[CUADRATURA] ${cruzados}/${sinCruzarR.rows.length} viajes cruzados con cargas Sigetra`);
  return cruzados;
}

async function actualizarConductorDesdecruce(nombre: string, viaje: any) {
  const existeR = await pool.query(`SELECT id FROM conductores_perfil WHERE nombre = $1 LIMIT 1`, [nombre]);
  if (existeR.rows.length > 0) return;

  await pool.query(`
    INSERT INTO conductores_perfil (nombre, contrato, total_jornadas, km_total, litros_total_ecu,
      rendimiento_promedio, horas_total_activo, primera_jornada, ultima_jornada)
    VALUES ($1, $2, 1, 0, 0, NULL, 0, $3, $3)
    ON CONFLICT DO NOTHING
  `, [nombre, viaje.contrato || "", viaje.fecha_inicio]);
}
