...
| BRAIN | `OperativeBrain` | Executive summary: multi-contract KPIs, 7-day trend, billing T-1 |
...
- **T-1 Reconstructor** (`t1-reconstructor.ts`): Post-hoc trip reconstruction from previous day GPS data. Analyzes full day of `gps_unificado` per truck, identifies geocerca dwells (30+ min), builds billable journeys. Round trips (CD→City→CD) = 1 tarifa. Min distance 40km. Features: parada filtering (Copec/estacionamientos/hosterías skip as intermediate stops, not trip endpoints), retorno detection (non-CD→CD trips = retorno, not billed), flexible tariff matching with city equivalences (Chillán↔CD Chillán, CT Concepción↔Concepción, etc.), atomic transactions. Runs at 05:00 daily + first agent cycle. Currently 100% tariff match (28/28 viajes after filtering). Non-Cencosud destinations (BODEGAS SAN FRANCISCO, Prolesur, Watts, etc.) and inter-city transits without tariffs are auto-discarded. Inserts `fuente_viaje='T1_RECONSTRUCTOR'`, `estado='FACTURADO'|'PENDIENTE'`. API: `POST /api/cencosud/t1-reconstruir`, `GET /api/cencosud/t1-resultado?fecha=YYYY-MM-DD`
- **Geocerca Zona mappings**: KML zones renamed — #0=Ovalle, #6=Linares, #8=Molina, #10=Coquimbo, #13=CD Noviciado, #14=Concepción
